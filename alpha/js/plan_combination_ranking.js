...
class PC_ranking {
    ...
    updateProgressBar() {
        let add = this.plans_number * 3 / 1;
        let width = document.getElementById("PC_ranking_progress_bar").style.width;
        width = parseFloat(width);
        width += add;
        document.getElementById("PC_ranking_progress_bar").style.width = width + '%';
        document.getElementById("PC_ranking_progress_bar").render('progress');
    }

    ranking() {
        let progress = 0;
        let timer = setInterval(this.updateProgressBar, 30);
        //第一次
